...
*/    
    ...
    case RFLR_STATE_RX_RUNNING:
        SX1276Read(REG_LR_IRQFLAGS, &dat);
        if (dat & RFLR_IRQFLAGS_RXDONE) {
        // if( DIO0 != RESET ) { // RxDone
            RxTimeoutTimer = GET_TICK_COUNT( );
            if( LoRaSettings.FreqHopOn == true )
            {
                SX1276Read( REG_LR_HOPCHANNEL, &SX1276LR->RegHopChannel );
        ...
        //if( DIO2 != RESET ) { // FHSS Changed Channel
            RxTimeoutTimer = GET_TICK_COUNT( );
            if( LoRaSettings.FreqHopOn == true )
            {
                SX1276Read( REG_LR_HOPCHANNEL, &SX1276LR->RegHopChannel );
    ...
    case RFLR_STATE_TX_RUNNING:
        SX1276Read(REG_LR_IRQFLAGS, &dat);
        if (dat & RFLR_IRQFLAGS_TXDONE) {
        //if( DIO0 != RESET ) { // TxDone
            // Clear Irq
    ...
    case RFLR_STATE_CAD_RUNNING:
        SX1276Read(REG_LR_IRQFLAGS, &dat);
        if (dat & RFLR_IRQFLAGS_CADDONE) {
        // if( DIO3 != RESET ) { //CAD Done interrupt
            // Clear Irq
            SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_CADDONE  );
            if (dat & RFLR_IRQFLAGS_CADDETECTED) {
            //if( DIO4 != RESET ) { }// CAD Detected interrupt
                // Clear Irq
